    ...
    if student_form.is_valid():
        ...
        if remaining_activation_times <= 0:
            ...
            return simplejson.dumps({
                'field': student_form.data.keys(),
                'status': '1',
                'remaining_activation_times': remaining_activation_times,
                'message':message})
        else:
            flag = Send_email_to_student(request, username, None, email,
                                         category, person_name, STUDENT_USER)
            if flag:
                message = u"发送邮件成功"
        ...
        return simplejson.dumps({
            'field': student_form.data.keys(),
            'error_id': student_form.errors.keys(),
            'message': u"输入有误",
            'remaining_activation_times': remaining_activation_times})
